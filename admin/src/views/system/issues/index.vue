<template>
  <div>
    ...
    >
      ...
      <Column field="userId" header="Add By">
        <template #body="slotProps">
          <div>
            {{ slotProps.data.userId.firstName }} {{ slotProps.data.userId.lastName }}
          </div>
        </template>
      </Column>
      ...
      <Column field="id" bodyStyle="text-align: center; overflow: visible">
        <template #body="slotProps">
          <Button
            type="button"
            @click="$router.push('issues/show/' + slotProps.data._id)"
            icon="pi pi-eye"
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.$http.get(`issues?page=${this.page + 1}&limit=10000`).then(
        (response) => {
          this.loading = false;
    ...
    deleteItem(id) {
      this.$confirm.require({
        ...
        accept: () => {
          this.$http.delete(`issues/${id}`).then(
            () => {
              this.getData();
              this.$toast.add({
                severity: 'error',
